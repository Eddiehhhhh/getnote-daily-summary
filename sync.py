...
当前版本按轻量 harness 方式组织：
1. workflow：筛选录音 → 分析 → 查找日记 → 写回 Notion
2. state：每次运行写入 `.harness/` 状态和检查点
3. checkpoint：关键步骤显式记录输入、输出和状态
4. eval：支持用固定样例回归验证 AI 解析行为
"""
...
LATEST_RUN_PATH = HARNESS_DIR / "latest-run.json"
WORKFLOW_STATE_PATH = HARNESS_DIR / "workflow-state.json"

# 情绪模板：名称 → Notion page_id（作为模板页复制 blocks，再新建当天记录）
EMOTION_MAP = {
...
def ensure_harness_dirs():
    HARNESS_DIR.mkdir(exist_ok=True)
    RUNS_DIR.mkdir(exist_ok=True)


def build_run_context(target_date, start_time, end_time, dry_run=False):
    run_started_at = now_cn()
    run_id = run_started_at.strftime("%Y%m%dT%H%M%S")
    run_path = RUNS_DIR / f"{target_date}-{run_id}.json"
    return {
        "run_id": run_id,
...
def require_envs(*names):
    ...
    if missing:
        raise CheckpointError("缺少必要环境变量: " + ", ".join(missing))


def persist_run_context(ctx):
    ensure_harness_dirs()
    run_path = Path(ctx["run_path"])
    payload = dict(ctx)
    payload.pop("run_path", None)
    run_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    LATEST_RUN_PATH.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def record_checkpoint(ctx, name, status, **details):
    checkpoint = {
        "name": name,
        "status": status,
        "at": now_iso(),
    ...
    persist_run_context(ctx)


def fail_checkpoint(ctx, name, message, **details):
    ctx["errors"].append({"name": name, "message": message, "details": details, "at": now_iso()})
    record_checkpoint(ctx, name, "failed", message=message, **details)
    raise CheckpointError(message)


def finalize_run_context(ctx, status, **metrics):
    ctx["status"] = status
        ...
        json.dumps(
            {
                ...
                "last_target_date": ctx["target_date"],
                "last_finished_at": ctx["finished_at"],
                "metrics": ctx["metrics"],
            },
            ensure_ascii=False,
            indent=2,
...
2b. "health_detail": 健康补充说明。用户如果在说完“健康”后又补充了展开内容，这里要提取出来，尽量保留口语原意，没提到则为 null。
...
def analyze_with_ai(original_text):
    ...
    if result.get("health") and result["health"] not in HEALTH_OPTIONS:
        result["health"] = None
    if result.get("health_detail"):
        if not isinstance(result["health_detail"], str):
            result["health_detail"] = None
        else:
            result["health_detail"] = result["health_detail"].strip()[:120] or None
    ...
    sleep_info = result.get("sleep") or {}
    ...
    print(f'[INFO] 分析结果: score={result.get("score")} health={result.get("health")} '
          f'health_detail={result.get("health_detail")} emotion=[{emotion_str}] banana={result.get("banana")} '
...
def validate_analysis_shape(analysis):
    if not isinstance(analysis, dict):
        raise CheckpointError("AI 分析结果不是对象")
    if analysis.get("emotion") is not None and not isinstance(analysis["emotion"], list):
        raise CheckpointError("emotion 不是数组")
    if analysis.get("sleep") is not None and not isinstance(analysis["sleep"], dict):
        raise CheckpointError("sleep 不是对象")
    meaningful_fields = [
        "score",
        "health",
        "health_detail",
    ...
    hit_count = sum(1 for key in meaningful_fields if analysis.get(key) not in (None, "", [], {}))
    if hit_count == 0:
        raise CheckpointError("AI 分析没有产出任何有效字段")
    return {"meaningful_field_count": hit_count}


def run_eval_cases(eval_fixture_path):
    fixture_path = Path(eval_fixture_path)
    cases = json.loads(fixture_path.read_text(encoding="utf-8"))
    if not isinstance(cases, list) or not cases:
        raise SystemExit("eval fixture must be a non-empty JSON array")
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        result = analyze_with_ai(original_text)
        ...
        for key, expected_value in expected.items():
            ...
            if isinstance(expected_value, list):
                if sorted(actual_value or []) != sorted(expected_value):
                    case_failures.append(f"{key}: expected {expected_value}, got {actual_value}")
            else:
                if actual_value != expected_value:
                    case_failures.append(f"{key}: expected {expected_value}, got {actual_value}")

        if case_failures:
            failures.append({"name": name, "failures": case_failures, "result": result})
        else:
            passed += 1

    summary = {"fixture": str(fixture_path), "passed": passed, "failed": len(failures), "total": len(cases)}
...
def update_existing_record(record_id, props, dry_run=False):
    """更新一条已有 Notion 页面的属性"""
    if dry_run:
        return
...
def update_title_record(record_id, db_name, title_field, new_title, dry_run=False):
    """更新已有关联记录的标题"""
...
def get_or_create_emotion_page(emotion_name, template_id, target_date, dry_run=False):
    """查找或创建情绪记录（使用模板结构）
...
def update_notion_page(page_id, analysis, target_date, dry_run=False):
    update_props = {}
    action_summary = {
        "page_id": page_id,
        "page_updates": [],
    ...
    action_summary["relations"] = {key: len(value) for key, value in rels.items()}

    # 评分
    if analysis.get("score"):
        update_props["评分"] = {"select": {"name": analysis["score"]}}
        action_summary["page_updates"].append("评分")

    # 感恩日记 - 更新已关联记录的标题
    if analysis.get("gratitude") and rels["gratitude"]:
        action_summary["record_updates"].append("感恩日记标题")
        update_title_record(
            rels["gratitude"][0]["id"], "感恩日记", "感恩日记", analysis["gratitude"], dry_run=dry_run
        )

    # 成功日记 - 更新已关联记录的标题
    if analysis.get("success") and rels["success"]:
        action_summary["record_updates"].append("成功日记标题")
        update_title_record(
            rels["success"][0]["id"], "成功日记", "名称", analysis["success"], dry_run=dry_run
        )
    ...
    if analysis.get("summary"):
        ...
        action_summary["page_updates"].append("总结")

    # 🍌
    if analysis.get("banana") is not None:
        update_props["🍌"] = {"checkbox": analysis["banana"]}
        action_summary["page_updates"].append("🍌")
    ...
    if analysis.get("emotion"):
        ...
        for emotion_name in analysis["emotion"]:
            ...
            if template_id:
                emotion_page_id = get_or_create_emotion_page(emotion_name, template_id, target_date, dry_run=dry_run)
                emotion_relations.append({"id": emotion_page_id})
        if emotion_relations:
            update_props["情绪"] = {"relation": emotion_relations}
            action_summary["page_updates"].append(f"情绪({len(emotion_relations)})")
    ...
    if analysis.get("health") and rels["health"]:
        ...
        try:
            health_page = notion_get(f"/pages/{record_id}")
            title_field = get_title_field_name(health_page)
            health_title = analysis["health"]
            if analysis.get("health_detail"):
                health_title = f"{analysis['health']}｜{analysis['health_detail']}"
            update_existing_record(
                record_id,
                {
                    "单选": {"select": {"name": analysis["health"]}},
                    title_field: {"title": [{"type": "text", "text": {"content": health_title[:100]}}]},
                },
                dry_run=dry_run,
            )
            action_summary["record_updates"].append("健康记录")
        except Exception as e:
    ...
    if analysis.get("sleep") and rels["sleep"]:
        ...
        if any([quality, energy, dreams]):
            ...
            try:
                update_existing_record(record_id, props, dry_run=dry_run)
                action_summary["record_updates"].append("睡眠记录")
            except Exception as e:
    ...
    if dry_run:
        print("[INFO] dry-run 模式：跳过实际写入日记页面")
    else:
...
# ============ 主流程 ============
def process_date(target_date, start_time, end_time, notes, ctx, dry_run=False):
    """处理某个日期的录音"""
    daily_notes = find_daily_notes(notes, start_time, end_time)
    if not daily_notes:
        record_checkpoint(ctx, "note_selection", "skipped", candidate_count=0)
        return False
    ...
    record_checkpoint(
        ...
        "note_selection",
        "passed",
        candidate_count=len(daily_notes),
        note_ids=[note["note_id"] for note in daily_notes],
        matched_by=[note["matched_by"] for note in daily_notes],
    )
    ...
    if not all_original:
        fail_checkpoint(ctx, "transcript_merge", "所有录音原文为空")
        return False

    combined_text = "\n\n".join(all_original)
    print(f"[INFO] 合并原文共 {len(combined_text)} 字符")
    record_checkpoint(ctx, "transcript_merge", "passed", transcript_count=len(all_original), combined_length=len(combined_text))

    # AI 分析
    analysis = analyze_with_ai(combined_text)
    analysis_metrics = validate_analysis_shape(analysis)
    record_checkpoint(ctx, "analysis", "passed", analysis=analysis, **analysis_metrics)

    # 找到 Notion 页面
    page_id = find_notion_page(target_date)
    if not page_id:
        fail_checkpoint(ctx, "notion_lookup", f"找不到 {target_date} 的日记页面", target_date=target_date)
        return False
    record_checkpoint(ctx, "notion_lookup", "passed", page_id=page_id)

    # 写入 Notion
    action_summary = update_notion_page(page_id, analysis, target_date, dry_run=dry_run)
    record_checkpoint(ctx, "notion_write", "passed", action_summary=action_summary, dry_run=dry_run)
    return True


def main():
    parser = argparse.ArgumentParser(description="Get 每日总结 -> Notion harness workflow")
    parser.add_argument("--dry-run", action="store_true", help="运行完整流程但不写入 Notion")
    parser.add_argument("--eval-fixture", help="运行固定样例评估，JSON array")
    args = parser.parse_args()

    if args.eval_fixture:
        require_envs("DEEPSEEK_API_KEY")
        run_eval_cases(args.eval_fixture)
        return
    ...
    ctx = build_run_context(target_date, start, end, dry_run=args.dry_run)
    try:
        require_envs("GETNOTE_API_KEY", "GETNOTE_CLIENT_ID", "NOTION_TOKEN", "NOTION_DB_ID", "DEEPSEEK_API_KEY")
        record_checkpoint(ctx, "preflight", "passed", mode="sync", dry_run=args.dry_run)
        print(f"[INFO] 目标日期: {target_date}，检查范围: {start} ~ {end}")

        # 拉取所有笔记
        notes = fetch_getnote_notes()
        ctx["metrics"]["fetched_note_count"] = len(notes)
        record_checkpoint(ctx, "fetch_notes", "passed", fetched_note_count=len(notes))

        if process_date(target_date, start, end, notes, ctx, dry_run=args.dry_run):
            finalize_run_context(ctx, "success", processed=True)
            print("[INFO] ✅ 全部完成！")
        else:
            finalize_run_context(ctx, "skipped", processed=False)
            print("[INFO] 没有需要处理的录音，跳过。")
    except CheckpointError as exc:
        finalize_run_context(ctx, "failed", error=str(exc))
        print(f"[ERROR] {exc}", file=sys.stderr)
        raise SystemExit(1)
    except Exception as exc:
        ctx["errors"].append({"name": "unexpected", "message": str(exc), "at": now_iso()})
        finalize_run_context(ctx, "failed", error=str(exc))
        raise
